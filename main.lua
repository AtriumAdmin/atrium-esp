...
local Drawing = Drawing or loadstring(game.HttpGet(game, "https://raw.githubusercontent.com/AtriumAdmin/atrium-esp/main/config.lua"))();
...
local Load = function()
    local Succ, Settings = pcall(JSONDecode, Services.HttpService, readfile("atrium-esp.json"));
    if (type(Settings) ~= 'table') then
        delfile("atrium-esp.json");
        return nil
    end
    local NewSettings = {}
...
local Settings = isfile("atrium-esp.json") and Load() or {
    TracerOptions = {
        Enabled = true,
        To = "Head",
        From = "Bottom",
...
local Save = function()
    if ((tick() - Debounce) >= 1.5) then
        ...
        writefile("atrium-esp.json", JSONEncode(Services.HttpService, NewSettings));
        Settings = Load();
...
Window = UI:LoadWindow('<font color="#ff4f57">atrium</font> esp', UDim2.fromOffset(400, 279));
...
if (not isfile("atrium-esp.json")) then
    Save();
end
